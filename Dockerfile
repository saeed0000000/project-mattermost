...
RUN --mount=type=cache,target=/root/.npm \
    npm rebuild gifsicle mozjpeg optipng-bin pngquant-bin
...
# Ensure a default config exists for packaging (cache go build and modules).
RUN --mount=type=cache,target=/go/pkg/mod \
    --mount=type=cache,target=/root/.cache/go-build \
    set -eux; \
    if [ ! -f config/config.json ]; then \
      OUTPUT_CONFIG=/src/server/config/config.json go run ./scripts/config_generator; \
    fi

# Assemble a runtime filesystem layout similar to official packaging
RUN set -eux; \
    mkdir -p /out/layout/bin \
             /out/layout/client \
             /out/layout/config \
             /out/layout/fonts \
             /out/layout/i18n \
